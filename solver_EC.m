function [solution,i_profiles]=solver_EC(ReactionParameters,ElectricalParameters,Ctrl,Const,x,t)
    m = 0; 
    options = odeset('AbsTol',Const.AbsTol);
   solution = pdepe(m,@pdex4pde,@pdex4ic,@pdex4bc,x,t,options);% i_profiles
    sizes=size(solution);
    for i=1:sizes(1)
...
i_profiles=Const.Faraday*Const.DiffusionCo*derivitive;
   ...
   s = [0;-ReactionParameters.k_EC*u(2);ReactionParameters.k_EC*u(2)];
   end
   function u0 = pdex4ic(x)
  u0 = [ReactionParameters.Concentration_Re_mmolPerLiter; ReactionParameters.Concentration_Ox_mmolPerLiter;0]; 
% --------------------------------------------------------------
   end
...
% constants and variables
 % over potential
  ...
  pr=[0;0;0];
  qr=[1;1;1];
%   pr = [ur(1)-ReactionParameters.Concentration_Re_mmolPerLiter;ur(2)-ReactionParameters.Concentration_Ox_mmolPerLiter;ur(3) ]; 
%   qr = [0; 0;0];
   end
